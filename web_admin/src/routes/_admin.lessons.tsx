...
function LessonsPage() {
  ...
  const lessons = useQuery({ 
    queryKey: ["contents", facilityId ?? "all"], 
    queryFn: () => contentApi.list(facilityId) 
  });
...
// Updated with audio/video support and file size validation
...
function LessonModal({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const maxSize = form.type === "video" ? 150 * 1024 * 1024 : 10 * 1024 * 1024; // 150MB for video, 10MB for others

    const fileTypeName = form.type === "video" ? "Video" : form.type === "audio" ? "Audio" : "Document";

    const maxSizeMB = form.type === "video" ? "150MB" : "10MB";



    if (selectedFile.size > maxSize) {

      setFileError(`${fileTypeName} file size must not exceed ${maxSizeMB}`);

      setFile(undefined);

      toast.error(`File too large! ${fileTypeName} files must be under ${maxSizeMB}`);
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (form.type !== "text" && !file && !editing?.fileUrl && !form.fileUrl) {

      toast.error(`Please upload a ${form.type} file or provide a file URL`);
    ...
    <Modal
      ...
      footer={
        ...
        <>
          ...
          <Button onClick={submit} disabled={saving || !!fileError}>
      ...
      <form onSubmit={submit} className="space-y-4">
          ...
          <FormField label="Content type *">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setForm({ ...form, type: e.target.value as ContentType, fileUrl: undefined, textContent: undefined });
        ...
        {form.type === "text" ? (
          ...
          <>
            ...
            <FormField label="Or upload a document (PDF / DOCX)" hint="optional · max 10MB">
          ...
          <div className="space-y-4">
            ...
            <FormField 
              ...
              label={`Upload ${form.type === "audio" ? "audio file" : "video file"} *`}

              hint={form.type === "video" ? "max 150MB" : "max 10MB"}
            ...
            >
              ...
              <input
                ...
                accept={form.type === "audio" ? "audio/*,.mp3,.wav,.m4a,.aac,.ogg" : "video/*,.mp4,.webm,.ogg,.mov"}
              ...
              <button
                ...
                className={`${inputClass} flex items-center gap-2 cursor-pointer ${
                  ...
                  fileError ? "border-destructive" : file ? "text-foreground" : "text-muted-foreground hover:text-foreground"
