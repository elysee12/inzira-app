...
export default function ParentChatScreen() {
  ...
  const handleNewChat = () => {
    console.log("Parent handleNewChat - chw:", chw);
    if (chw) {
      console.log("Parent handleNewChat - navigating to:", `/(parent)/chat-conversation/${chw.id}`);
      router.push({
        pathname: "/(parent)/chat-conversation/[userId]",
        params: { userId: chw.id, userName: chw.name }
      });
    } else {
      Alert.alert("Ntacyo bishoboye", "Nta mukozi w'ubuzima wakuhawe.");
    }
  };
        ...
        {conversations.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={[styles.emptyHint, { color: colors.mutedForeground }]}>
              Tangira uganira n'umukozi w'ubuzima waguhawe
            </Text>
            <TouchableOpacity
