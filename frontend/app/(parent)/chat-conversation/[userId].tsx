...
export default function ChatConversationScreen() {
  ...
  const fetchConversation = async () => {
    try {
      ...
      } else if (response.data.length === 0 && !otherUser) {
        // No messages yet, try to get assigned CHW
        try {
          const chwResponse = await apiClient.get(`/users/${currentUserId}/chw`);
          if (chwResponse.data) {
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerInfo}>
            <Text style={[styles.headerTitle, { color: colors.foreground }]}>
              {otherUser?.name || "Umukozi w'Ubuzima"}
            </Text>
